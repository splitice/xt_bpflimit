...
#define XT_BPFLIMIT_BURST_MAX_v1	10000
#define XT_BPFLIMIT_BURST_MAX		1000000
...
static void bpflimit_mt_help_v3(void)
{
	printf(
"bpflimit match options:\n"
"  --bpflimit-upto <avg>           max average match rate\n"
...
"  --bpflimit-htable-gcinterval    interval between garbage collection runs\n"
"  --bpflimit-htable-expire        after which time are idle entries expired?\n"
"  --bpflimit-rate-match           rate match the flow without rate-limiting it\n"
"  --bpflimit-rate-interval        interval in seconds for bpflimit-rate-match\n"
"\n", XT_BPFLIMIT_BURST);
}
...
static const struct xt_option_entry bpflimit_opts[] = {
	...
	{.name = "bpflimit-burst", .id = O_BURST, .type = XTTYPE_UINT32,
	 .min = 1, .max = XT_BPFLIMIT_BURST_MAX_v1, .flags = XTOPT_PUT,
...
static const struct xt_option_entry bpflimit_mt_opts_v1[] = {
...
#define s struct xt_bpflimit_mtinfo2
static const struct xt_option_entry bpflimit_mt_opts_v2[] = {
	{.name = "bpflimit-upto", .id = O_UPTO, .excl = F_ABOVE,
	 .type = XTTYPE_STRING, .flags = XTOPT_INVERT},
	{.name = "bpflimit-above", .id = O_ABOVE, .excl = F_UPTO,
	 .type = XTTYPE_STRING, .flags = XTOPT_INVERT},
...
#define s struct xt_bpflimit_mtinfo3
static const struct xt_option_entry bpflimit_mt_opts[] = {
	...
	{.name = "bpflimit-name", .id = O_NAME, .type = XTTYPE_STRING,
	 .flags = XTOPT_MAND | XTOPT_PUT, XTOPT_POINTER(s, name), .min = 1},
	{.name = "bpflimit-rate-match", .id = O_RATEMATCH, .type = XTTYPE_NONE},
	{.name = "bpflimit-rate-interval", .id = O_INTERVAL, .type = XTTYPE_STRING},
	XTOPT_TABLEEND,
};
#undef s

static int
cfg_copy(struct bpflimit_cfg3 *to, const void *from, int revision)
{
	if (revision == 1) {
		struct bpflimit_cfg1 *cfg = (struct bpflimit_cfg1 *)from;
		...
		struct bpflimit_cfg2 *cfg = (struct bpflimit_cfg2 *)from;
		...
		memcpy(to, from, sizeof(struct bpflimit_cfg3));
	} else {
		return -EINVAL;
	}
...
static void burst_error_v1(void)
{
	xtables_error(PARAMETER_PROBLEM, "bad value for option "
			"\"--bpflimit-burst\", or out of range (1-%u).", XT_BPFLIMIT_BURST_MAX_v1);
}
...
static uint64_t parse_burst(const char *burst, int revision)
{
	...
	uint64_t burst_max = (revision == 1) ?
			      XT_BPFLIMIT_BURST_MAX_v1 : XT_BPFLIMIT_BURST_MAX;
...
static bool parse_bytes(const char *rate, void *val, struct bpflimit_mt_udata *ud, int revision)
{
...
static
int parse_rate(const char *rate, void *val, struct bpflimit_mt_udata *ud, int revision)
{
	const char *delim;
	uint64_t tmp, r;
	uint64_t scale = (revision == 1) ? XT_BPFLIMIT_SCALE : XT_BPFLIMIT_SCALE_v2;
...
static void bpflimit_mt4_init_v1(struct xt_entry_match *match)
{
...
static void bpflimit_mt6_init_v1(struct xt_entry_match *match)
{
	struct xt_bpflimit_mtinfo1 *info = (void *)match->data;
...
static void bpflimit_mt4_init_v2(struct xt_entry_match *match)
{
	struct xt_bpflimit_mtinfo2 *info = (void *)match->data;
...
static void bpflimit_mt6_init_v2(struct xt_entry_match *match)
{
	struct xt_bpflimit_mtinfo2 *info = (void *)match->data;
...
{
	struct xt_bpflimit_mtinfo3 *info = (void *)match->data;
...
static void bpflimit_mt_parse_v1(struct xt_option_call *cb)
{
...
static void bpflimit_mt_parse_v2(struct xt_option_call *cb)
{
	struct xt_bpflimit_mtinfo2 *info = cb->data;
...
{
	struct xt_bpflimit_mtinfo3 *info = cb->data;
	...
	case O_DSTMASK:
		...
		break;
	case O_RATEMATCH:
		info->cfg.mode |= XT_BPFLIMIT_RATE_MATCH;
		break;
	case O_INTERVAL:
		if (!parse_interval(cb->arg, &info->cfg.interval))
			xtables_param_act(XTF_BAD_VALUE, "bpflimit",
				"--bpflimit-rate-interval", cb->arg);
	}
}
...
static void bpflimit_mt_check_v1(struct xt_fcheck_call *cb)
{
	const struct bpflimit_mt_udata *udata = cb->udata;
	struct xt_bpflimit_mtinfo1 *info = cb->data;
	...
	} else if (info->cfg.burst > XT_BPFLIMIT_BURST_MAX_v1)
		burst_error_v1();
}

static void bpflimit_mt_check_v2(struct xt_fcheck_call *cb)
{
	const struct bpflimit_mt_udata *udata = cb->udata;
	struct xt_bpflimit_mtinfo2 *info = cb->data;
...
struct rates {
	const char *name;
	uint64_t mult;
} rates_v1[] = { { "day", XT_BPFLIMIT_SCALE*24*60*60 },
		 { "hour", XT_BPFLIMIT_SCALE*60*60 },
		 { "min", XT_BPFLIMIT_SCALE*60 },
		 { "sec", XT_BPFLIMIT_SCALE } };

static const struct rates rates[] = {
	{ "day", XT_BPFLIMIT_SCALE_v2*24*60*60 },
	{ "hour", XT_BPFLIMIT_SCALE_v2*60*60 },
	{ "min", XT_BPFLIMIT_SCALE_v2*60 },
	{ "sec", XT_BPFLIMIT_SCALE_v2 } };
...
static void
bpflimit_mt_print(const struct bpflimit_cfg3 *cfg, unsigned int dmask, int revision)
{
	uint64_t quantum;
	uint64_t period;

	if (cfg->mode & XT_BPFLIMIT_INVERT)
		fputs(" limit: above", stdout);
	else
		fputs(" limit: up to", stdout);

	if (cfg->mode & XT_BPFLIMIT_BYTES) {
	...
	if (cfg->mode & (XT_BPFLIMIT_HASH_SIP | XT_BPFLIMIT_HASH_SPT |
	...
	if (cfg->gc_interval != XT_BPFLIMIT_GCINTERVAL)
	...
	if ((revision == 3) && (cfg->mode & XT_BPFLIMIT_RATE_MATCH))
		printf(" rate-match");
...
static void
bpflimit_mt4_print_v1(const void *ip, const struct xt_entry_match *match,
                   int numeric)
{
	...
	bpflimit_mt_print(&cfg, 32, 1);
}

static void
bpflimit_mt6_print_v1(const void *ip, const struct xt_entry_match *match,
                   int numeric)
{
	const struct xt_bpflimit_mtinfo1 *info = (const void *)match->data;
	struct bpflimit_cfg3 cfg;
	...
	bpflimit_mt_print(&cfg, 128, 1);
}

static void
bpflimit_mt4_print_v2(const void *ip, const struct xt_entry_match *match,
                   int numeric)
{
	const struct xt_bpflimit_mtinfo2 *info = (const void *)match->data;
	struct bpflimit_cfg3 cfg;
	int ret;
	...
	bpflimit_mt_print(&cfg, 32, 2);
}

static void
bpflimit_mt6_print_v2(const void *ip, const struct xt_entry_match *match,
                   int numeric)
{
	const struct xt_bpflimit_mtinfo2 *info = (const void *)match->data;
	struct bpflimit_cfg3 cfg;
	...
	bpflimit_mt_print(&cfg, 128, 2);
}
static void
bpflimit_mt4_print(const void *ip, const struct xt_entry_match *match,
                   int numeric)
{
	const struct xt_bpflimit_mtinfo3 *info = (const void *)match->data;

	bpflimit_mt_print(&info->cfg, 32, 3);
}
                   ...
                   int numeric)
{
	...
	bpflimit_mt_print(&info->cfg, 128, 3);
}
...
static void
bpflimit_mt_save(const struct bpflimit_cfg3 *cfg, const char* name, unsigned int dmask, int revision)
{
	uint32_t quantum;
	...
	if (cfg->mode & XT_BPFLIMIT_BYTES) {
		quantum = print_bytes(cfg->avg, cfg->burst, "--bpflimit-");
	} else {
		quantum = print_rate(cfg->avg, revision);
		printf(" --bpflimit-burst %llu", cfg->burst);
	}
	...
	printf(" --bpflimit-name %s", name);

	if (cfg->size != 0)
		printf(" --bpflimit-htable-size %u", cfg->size);
	if (cfg->max != 0)
		printf(" --bpflimit-htable-max %u", cfg->max);
	if (cfg->gc_interval != XT_BPFLIMIT_GCINTERVAL)
		printf(" --bpflimit-htable-gcinterval %u", cfg->gc_interval);
	if (cfg->expire != quantum)
		printf(" --bpflimit-htable-expire %u", cfg->expire);

	if (cfg->srcmask != dmask)
		printf(" --bpflimit-srcmask %u", cfg->srcmask);
	if (cfg->dstmask != dmask)
		printf(" --bpflimit-dstmask %u", cfg->dstmask);

	if ((revision == 3) && (cfg->mode & XT_BPFLIMIT_RATE_MATCH))
		printf(" --bpflimit-rate-match");

	if ((revision == 3) && (cfg->mode & XT_BPFLIMIT_RATE_MATCH))
		if (cfg->interval != 1)
			printf(" --bpflimit-rate-interval %u", cfg->interval);
}

static void
bpflimit_mt4_save_v1(const void *ip, const struct xt_entry_match *match)
{
	const struct xt_bpflimit_mtinfo1 *info = (const void *)match->data;
	struct bpflimit_cfg3 cfg;
	int ret;
	...
	bpflimit_mt_save(&cfg, info->name, 32, 1);
}

static void
bpflimit_mt6_save_v1(const void *ip, const struct xt_entry_match *match)
{
	const struct xt_bpflimit_mtinfo1 *info = (const void *)match->data;
	struct bpflimit_cfg3 cfg;
	int ret;
	...
	bpflimit_mt_save(&cfg, info->name, 128, 1);
}

static void
bpflimit_mt4_save_v2(const void *ip, const struct xt_entry_match *match)
{
	const struct xt_bpflimit_mtinfo2 *info = (const void *)match->data;
	struct bpflimit_cfg3 cfg;
	int ret;
	...
	bpflimit_mt_save(&cfg, info->name, 32, 2);
}

static void
bpflimit_mt6_save_v2(const void *ip, const struct xt_entry_match *match)
{
	const struct xt_bpflimit_mtinfo2 *info = (const void *)match->data;
	struct bpflimit_cfg3 cfg;
	int ret;
	...
	bpflimit_mt_save(&cfg, info->name, 128, 2);
}
...
bpflimit_mt4_save(const void *ip, const struct xt_entry_match *match)
{
	...
	bpflimit_mt_save(&info->cfg, info->name, 32, 3);
}
...
static const struct rates rates_v1_xlate[] = {
	...
	{ "hour", XT_BPFLIMIT_SCALE * 60 * 60 },
	{ "minute", XT_BPFLIMIT_SCALE * 60 },
	{ "second", XT_BPFLIMIT_SCALE } };

static const struct rates rates_xlate[] = {
	{ "day", XT_BPFLIMIT_SCALE_v2 * 24 * 60 * 60 },
	{ "hour", XT_BPFLIMIT_SCALE_v2 * 60 * 60 },
	{ "minute", XT_BPFLIMIT_SCALE_v2 * 60 },
	{ "second", XT_BPFLIMIT_SCALE_v2 } };
...
static void print_bytes_rate_xlate(struct xt_xlate *xl,
				   const struct bpflimit_cfg3 *cfg)
{
	unsigned int i;
	unsigned long long r;
...
static void bpflimit_print_subnet_xlate(struct xt_xlate *xl,
...
static const char *const bpflimit_modes4_xlate[] = {
	[XT_BPFLIMIT_HASH_DIP]	= "ip daddr",
	[XT_BPFLIMIT_HASH_DPT]	= "tcp dport",
	[XT_BPFLIMIT_HASH_SIP]	= "ip saddr",
	[XT_BPFLIMIT_HASH_SPT]	= "tcp sport",
};

static const char *const bpflimit_modes6_xlate[] = {
	[XT_BPFLIMIT_HASH_DIP]	= "ip6 daddr",
	[XT_BPFLIMIT_HASH_DPT]	= "tcp dport",
	[XT_BPFLIMIT_HASH_SIP]	= "ip6 saddr",
	[XT_BPFLIMIT_HASH_SPT]	= "tcp sport",
};

static int bpflimit_mode_xlate(struct xt_xlate *xl,
				uint32_t mode, int family,
				unsigned int nsrc, unsigned int ndst)
{
	const char * const *_modes = (family == NFPROTO_IPV4) ?
		bpflimit_modes4_xlate : bpflimit_modes6_xlate;
	bool prevopt = false;
	unsigned int mask;

	mode &= ~XT_BPFLIMIT_INVERT & ~XT_BPFLIMIT_BYTES;
	...
	for (mask = 1; mode > 0; mask <<= 1) {
		...
		if (mask == XT_BPFLIMIT_HASH_DIP &&
		    ((family == NFPROTO_IPV4 && ndst != 32) ||
		     (family == NFPROTO_IPV6 && ndst != 128)))
			bpflimit_print_subnet_xlate(xl, ndst, family);
		else if (mask == XT_BPFLIMIT_HASH_SIP &&
			 ((family == NFPROTO_IPV4 && nsrc != 32) ||
			  (family == NFPROTO_IPV6 && nsrc != 128)))
			bpflimit_print_subnet_xlate(xl, nsrc, family);
...
static int bpflimit_mt_xlate(struct xt_xlate *xl, const char *name,
			      const struct bpflimit_cfg3 *cfg,
			      int revision, int family)
{
	...
	ret = bpflimit_mode_xlate(xl, cfg->mode, family,
	...
	if (cfg->mode & XT_BPFLIMIT_BYTES)
		...
		if (cfg->burst != XT_BPFLIMIT_BURST)
...
{
	const struct xt_bpflimit_info *info = (const void *)params->match->data;
	int ret = 1;

	xt_xlate_add(xl, "meter %s {", info->name);
	ret = bpflimit_mode_xlate(xl, info->cfg.mode, NFPROTO_IPV4, 32, 32);
...
static int bpflimit_mt4_xlate_v1(struct xt_xlate *xl,
				  const struct xt_xlate_mt_params *params)
{
	const struct xt_bpflimit_mtinfo1 *info =
	...
	return bpflimit_mt_xlate(xl, info->name, &cfg, 1, NFPROTO_IPV4);
}

static int bpflimit_mt6_xlate_v1(struct xt_xlate *xl,
				  const struct xt_xlate_mt_params *params)
{
	const struct xt_bpflimit_mtinfo1 *info =
		(const void *)params->match->data;
	...
	return bpflimit_mt_xlate(xl, info->name, &cfg, 1, NFPROTO_IPV6);
}

static int bpflimit_mt4_xlate_v2(struct xt_xlate *xl,
				  const struct xt_xlate_mt_params *params)
{
	const struct xt_bpflimit_mtinfo2 *info =
		(const void *)params->match->data;
	struct bpflimit_cfg3 cfg;
	...
	return bpflimit_mt_xlate(xl, info->name, &cfg, 2, NFPROTO_IPV4);
}

static int bpflimit_mt6_xlate_v2(struct xt_xlate *xl,
				  const struct xt_xlate_mt_params *params)
{
	const struct xt_bpflimit_mtinfo2 *info =
		(const void *)params->match->data;
	...
	return bpflimit_mt_xlate(xl, info->name, &cfg, 2, NFPROTO_IPV6);
}

static int bpflimit_mt4_xlate(struct xt_xlate *xl,
			       const struct xt_xlate_mt_params *params)
{
	const struct xt_bpflimit_mtinfo3 *info =
		(const void *)params->match->data;

	return bpflimit_mt_xlate(xl, info->name, &info->cfg, 3, NFPROTO_IPV4);
}

static int bpflimit_mt6_xlate(struct xt_xlate *xl,
			       const struct xt_xlate_mt_params *params)
{
	const struct xt_bpflimit_mtinfo3 *info =
		(const void *)params->match->data;
...
//		.xlate         = bpflimit_xlate,
	},
	{
		...
		.init          = bpflimit_mt4_init_v1,
		.x6_parse      = bpflimit_mt_parse_v1,
		.x6_fcheck     = bpflimit_mt_check_v1,
		.print         = bpflimit_mt4_print_v1,
...
//		.xlate         = bpflimit_mt4_xlate_v1,
	},
	{
		...
		.userspacesize = offsetof(struct xt_bpflimit_mtinfo1, hinfo),
		.help          = bpflimit_mt_help,
		.init          = bpflimit_mt6_init_v1,
		.x6_parse      = bpflimit_mt_parse_v1,
		.x6_fcheck     = bpflimit_mt_check_v1,
		.print         = bpflimit_mt6_print_v1,
...
//		.xlate         = bpflimit_mt6_xlate_v1,
	},
	{
		...
		.revision      = 2,
		.family        = NFPROTO_IPV4,
		.size          = XT_ALIGN(sizeof(struct xt_bpflimit_mtinfo2)),
		.userspacesize = offsetof(struct xt_bpflimit_mtinfo2, hinfo),
		.help          = bpflimit_mt_help,
		.init          = bpflimit_mt4_init_v2,
		.x6_parse      = bpflimit_mt_parse_v2,
		.x6_fcheck     = bpflimit_mt_check_v2,
		.print         = bpflimit_mt4_print_v2,
...
//		.xlate         = bpflimit_mt4_xlate_v2,
	},
	{
		...
		.userspacesize = offsetof(struct xt_bpflimit_mtinfo2, hinfo),
		.help          = bpflimit_mt_help,
		.init          = bpflimit_mt6_init_v2,
		.x6_parse      = bpflimit_mt_parse_v2,
		.x6_fcheck     = bpflimit_mt_check_v2,
		.print         = bpflimit_mt6_print_v2,
...
//		.xlate         = bpflimit_mt6_xlate_v2,
	},
	{
		...
		.revision      = 3,
		.family        = NFPROTO_IPV4,
		.size          = XT_ALIGN(sizeof(struct xt_bpflimit_mtinfo3)),
		.userspacesize = offsetof(struct xt_bpflimit_mtinfo3, hinfo),
		.help          = bpflimit_mt_help_v3,
